...
$(function () {
	...
	// tab-nav
	var tabNavs = $('.tab-nav')
	var tabNavLens = $('.tab-nav li').length
	// if(tabNavLens < 4) tabNavs.addClass('lens'+ tabNavLens)
	
	$('.utilSet .btn-print').click(function(event) {
		window.print()
	});
	$('.header .logo a, .top-nav-user li:last-child a').on('focus', function(e){
		$('.lang-opener').removeClass('on');
	})
	$('.top-nav-user li:first-child a, .skip a').on('focus', function(e){
		$('.top-navlink>ul>li>a').removeClass('on');
	})
	...
	$('.table.ovx').before('<span class="mobile-scroller-x"></span>')
	...
	$(window).resize(function() {
		...
		window.resizedFinished = setTimeout(function(){
			if(window.innerWidth < 767){
				$('.table.ovx').addClass('tableovx-ico');
			} else {
				$('.table.ovx').removeClass('tableovx-ico');
			}
		}, 250);
	});
});
